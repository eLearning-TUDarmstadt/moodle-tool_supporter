...
$app->map('/users', 'tool_users')->via('GET');
...
$app->map('/users/indetail', 'moodleanalyst_usersInDetail')->via('GET');
*/
...
/**
 * Returns a JSON-Object with all Users in it
 */
function moodleanalyst_usersInDetail()
{
    /*
    ...
    $page = '';
    $recordsperpage = '100000000';
    $fields = 'id, username, firstname, lastname, email, lastaccess, lastlogin';
    $users = get_users($get, $search, $confirmed, $exceptions, $sort, $firstinitial, $lastinitial, $page, $recordsperpage, $fields);
    /*
    ...
    foreach ($users as $userid => $user) {
     ...
     mdl 
    */
   //moodleanalyst_printArray($users);
   echo json_encode($users);
}
...
function moodleanalyst_getVocabulary()
{
    ...
    $result['firstname'] = get_string('firstname');
    $result['lastname'] = get_string('lastname');
    $result['email'] = get_string('email');
    $result['lastlogin'] = get_string('lastlogin');
    $result['lastaccess'] = get_string('lastaccess');
...
function moodleanalyst_getActivitiesInCourse($courseid) {
    ...
    foreach ($activities as $modid => $activity) {
        ...
        //$table['rows'][] =
        //['c' => array('v' => $sectionname), array('v' => $activityType), array('v' => $activityname)];
